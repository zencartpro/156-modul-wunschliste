 ...
 * @version $Id: un_wishlist.php 2019-09-14 15:26:25Z webchills $
 */
 
// Include stuff
require_once 'un_productlist.php';
...
class un_wishlist extends un_productlist {
     ...
     * /*----------------------------------------------------------*@return bool|int
     */
    public function setWishlistId($iWishlistId=NULL) {
    ...
    /*----------------------------------------------------------
     */
	public function getDefaultWishlistId($iCustomerId=NULL) {
     ...
     *
     * @return array|bool * @access public
	* /*----------------------------------------------------------*/
	...
	* /*----------------------------------------------------------*/
    ...
    /*----------------------------------------------------------
     */
	public function getWishlistMenu($sName, $sDefault = '', $sParameters = '') {
    ...
    * /*----------------------------------------------------------
     */
    public function findWishlists($aArgs) {
        
		$sql = 'SELECT 
					w.id, w.name, w.comment, c.customers_id, c.customers_firstname, c.customers_lastname, c.customers_email_address, ab.entry_city, ab.entry_state 
				FROM 
					' .UN_TABLE_WISHLISTS. ' w, 
					' .TABLE_CUSTOMERS. ' c, 
...
/* 				dump($sql); exit; */
     ...
     *
     * @return array|string * @access public
    * /*----------------------------------------------------------
     */
     ...
     * @access private
	/*----------------------------------------------------------*/
    public function _createDefaultWishlist() {
    ...
    /*----------------------------------------------------------
     */
	public function _inWishlist($products_id) {
        ...
        return (int)$result->fields['items_count'] > 0;
    }
     ...
     * @param int $public_status
     * @return integer $iWishlistId unique wishlist identifier
     * @access public
    /*----------------------------------------------------------
     */
    public function createWishlist($name='', $comment='', $default_status=0, $public_status=1) {
     ...
     * @access public
	/*----------------------------------------------------------*/
    public function addWishlist($aArgs) {
     ...
     * @access public
	/*----------------------------------------------------------*/
    public function editWishlist($aArgs) {
    ...
    /*----------------------------------------------------------
     */
    public function addProduct($products_id, $attributes='', $quantity=1, $priority=2, $comment='') {
		//echo $attributes;
    ...
    * /*----------------------------------------------------------
     */
    public function wishlist_get_attributes($products_id) {
	    if ( $this->_iWishlistId ) {
    ...
    /*----------------------------------------------------------
     */
	public function updateProduct($products_id, $attributes='', $quantity=NULL, $priority=2, $comment='') {
    ...
    /*----------------------------------------------------------
     */
	public function moveProduct($products_id, $wishlists_id) {
     ...
     * not used yet
	/*----------------------------------------------------------*/
	public function _anyInWishlist() {
	...
	public function wishlist_get_products($check_for_valid_cart = false) {
    global $db;

    $this->notify('NOTIFIER_WISHLIST_GET_PRODUCTS_START');

    if (!$this->_anyInWishlist()) {
        return false;
    }
...
// validate cart contents for checkout
        if ($check_for_valid_cart == true) {
          ...
          if ($fix_once == 0 && $check_quantity < $check_quantity_min) {
            $fix_once ++;
            $_SESSION['valid_to_checkout'] = false;
        ...
        */
        ...
        if (false !== strpos($check_unit_decimals, '.')) {
          $new_qty = round($new_qty, QUANTITY_DECIMALS);
        } else {
